...
class ItemDialog extends Component {
  ...
  handleSubmit = async () => {
    ...
    const getData = await makeRequest(myURL, defaultData ? 'PUT' : 'POST', this.state.formData)
          .then(async resp=>{
              if(resp.ok) {
                return resp.json()
  ...
  render() {
    ...
    return (
      <div>
        <Button type="submit" color={this.props.color} variant={this.props.variant} onClick={this.handleClickOpen}>{defaultData ? "Edit Item" : "Create item" }</Button>
        ...
        >
          <DialogTitle id="alert-dialog-title">{defaultData ? defaultData.title : "New Article"}</DialogTitle>
